...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Blog: client-side search/filter and subscribe (simulated AJAX)
  (function blogEnhancements() {
    try {
      const searchForm = document.querySelector('form[role="search"]');
      const searchInput = document.getElementById("blog-search");
      const posts = Array.from(document.querySelectorAll("main article.card"));
      ...
      function filterPosts(q) {
        ...
        posts.forEach((post) => {
          ...
          const meta = (
            post.querySelector(".muted")?.textContent || ""
          ).toLowerCase();
          const cat = (post.dataset?.category || "").toLowerCase();
          if (title.includes(val) || meta.includes(val) || cat.includes(val)) {
            post.style.display = "";
          } else {
      ...
      // category quick-filter (aside lists)
      document.querySelectorAll("aside .card ul a").forEach((a) => {
        a.addEventListener("click", function (e) {
          e.preventDefault();
      ...
      // AJAX-like subscribe simulation
      const subscribeForm =
        document.querySelector('aside form[action="#"]') ||
      ...
      if (subscribeForm) {
        subscribeForm.addEventListener("submit", function (e) {
          ...
          setTimeout(() => {
            ...
            subscribeForm.appendChild(msg);
          }, 900);
        });
      }
    } catch (err) {
      console.warn("blog enhancements failed", err);
    }
  })();
});
